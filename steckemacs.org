#+Title: Steckemacs - ayoshi
#+Author: ayoshi

* DONE Options                                             :noexport:ARCHIVE:
...
*steckemacs-ayoshi* is an Emacs configuration based on an original config by Steckerhalter http://steckerhalter.co.vu/. The main diferrence from an original is that this configuration tries to replace VIM, and uses evil mode heavily. The goal is to make Emacs a much better VIM.

The second goal is to keep everything in one file =steckemacs.org= (plus =.emacs= for the initialization) and not as many others to split it over lots of files and directories. Packages are retrieved from the Emacs package repositories on the internet on the first startup. 
...
I use both sometimes, they are similar though ag is written in C and is considerably faster
...
(defun setup-php-mode ()
  (require 'tempo nil t) ;needed by php-documentor
